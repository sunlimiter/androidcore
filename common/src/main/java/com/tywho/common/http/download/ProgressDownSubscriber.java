...
import com.tywho.common.http.download.interceptor.DownloadProgressListener;
...
public class ProgressDownSubscriber<T> extends Subscriber<T> implements DownloadProgressListener {
    //弱引用结果回调
    private SoftReference<HttpDownCallBackListener> mSubscriberOnNextListener;
    /*下载数据*/
    private DownInfoBean downInfo;


    public ProgressDownSubscriber(DownInfoBean downInfo) {
        this.mSubscriberOnNextListener = new SoftReference<>(downInfo.getListener());
        this.downInfo = downInfo;
    }


    public void setDownInfo(DownInfoBean downInfo) {
        this.mSubscriberOnNextListener = new SoftReference<>(downInfo.getListener());
        this.downInfo = downInfo;
    }
...
//        DbDwonUtil.getInstance().update(downInfo);
    ...
    public void update(long read, long count, boolean done) {
        ...
        if (done) {
            String filename;
            if(StringUtils.isEmpty(downInfo.getSaveName())){
                filename = downInfo.getUrl().substring(downInfo.getUrl().lastIndexOf('/') + 1);
            }else{
                filename = downInfo.getSaveName();
            }

            new File(downInfo.getSavePath(),filename+"_temp").renameTo(new File(downInfo.getSavePath(),filename));
        }
        if (mSubscriberOnNextListener.get() != null) {
