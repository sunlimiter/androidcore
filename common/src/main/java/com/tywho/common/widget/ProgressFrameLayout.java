...
/**
 * Created by limit on 2017/4/13/0013.
 * 加载状态界面
 * from https://github.com/Kennyc1012/MultiStateView
 */
...
public class ProgressFrameLayout extends FrameLayout {
    ...
    public static final int VIEW_STATE_CONTENT_LOADING = 4;

    @Retention(RetentionPolicy.SOURCE)
    @IntDef({VIEW_STATE_UNKNOWN, VIEW_STATE_CONTENT, VIEW_STATE_ERROR, VIEW_STATE_EMPTY, VIEW_STATE_LOADING,VIEW_STATE_CONTENT_LOADING})
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.ProgressFrameLayout);

        int loadingViewResId = a.getResourceId(R.styleable.ProgressFrameLayout_msv_loadingView, R.layout.progress_frame_layout_loading_view);
        if (loadingViewResId > -1) {
            mLoadingView = mInflater.inflate(loadingViewResId, this, false);
            addView(mLoadingView, mLoadingView.getLayoutParams());
        }

        int emptyViewResId = a.getResourceId(R.styleable.ProgressFrameLayout_msv_emptyView, R.layout.progress_frame_layout_empty_view);
        if (emptyViewResId > -1) {
            mEmptyView = mInflater.inflate(emptyViewResId, this, false);
            addView(mEmptyView, mEmptyView.getLayoutParams());
        }

        int errorViewResId = a.getResourceId(R.styleable.ProgressFrameLayout_msv_errorView, R.layout.progress_frame_layout_error_view);
        if (errorViewResId > -1) {
            mErrorView = mInflater.inflate(errorViewResId, this, false);
            addView(mErrorView, mErrorView.getLayoutParams());
        }

        int viewState = a.getInt(R.styleable.ProgressFrameLayout_msv_viewState, VIEW_STATE_LOADING);
        mAnimateViewChanges = a.getBoolean(R.styleable.ProgressFrameLayout_msv_animateViewChanges, false);
        ...
        switch (viewState) {
            ...
            case VIEW_STATE_CONTENT_LOADING:
                mViewState = VIEW_STATE_CONTENT_LOADING;
                break;
    ...
    /**
     * Returns the {@link View} associated with the {@link com.tywho.common.widget.ProgressFrameLayout.ViewState}
     *
     * @param state The {@link com.tywho.common.widget.ProgressFrameLayout.ViewState} with to return the view for
     * @return The {@link View} associated with the {@link com.tywho.common.widget.ProgressFrameLayout.ViewState}, null if no view is present
     */
    @Nullable
    ...
    /**
     * Returns the current {@link com.tywho.common.widget.ProgressFrameLayout.ViewState}
     *
     * @return
     */
    @ViewState
    ...
    /**
     * Sets the current {@link com.tywho.common.widget.ProgressFrameLayout.ViewState}
     *
     * @param state The {@link com.tywho.common.widget.ProgressFrameLayout.ViewState} to set {@link ProgressFrameLayout} to
     */
    ...
    /**
     * Shows the {@link View} based on the {@link com.tywho.common.widget.ProgressFrameLayout.ViewState}
     */
    private void setView(@ViewState int previousState) {
        switch (mViewState) {
            ...
            case VIEW_STATE_CONTENT_LOADING:
                ...
                mLoadingView.setVisibility(View.VISIBLE);
                ObjectAnimator anim = ObjectAnimator.ofFloat(mLoadingView, "alpha", 1.0f, 0.0f).setDuration(250L);
                anim.start();
...
//                updateViewLayout(mLoadingView, mLoadingView.getLayoutParams());
                break;
            case VIEW_STATE_EMPTY:
                if (mEmptyView == null) {
     ...
     * @param state         The {@link com.tywho.common.widget.ProgressFrameLayout.ViewState}to set
     * @param switchToState If the {@link com.tywho.common.widget.ProgressFrameLayout.ViewState} should be switched to
     */
    ...
    /**
     * Sets the {@link View} for the given {@link com.tywho.common.widget.ProgressFrameLayout.ViewState}
     *
     * @param view  The {@link View} to use
     * @param state The {@link com.tywho.common.widget.ProgressFrameLayout.ViewState} to set
     */
    ...
    @BindingAdapter(value = {"state", "errorTryListener","emptyListener", "errorMsg", "emptyMsg", "errorIcon", "emptyIcon"}, requireAll = false)
    public static void showContent(ProgressFrameLayout view, int state,
                                   OnClickListener onClickListener,OnClickListener emptyListener, String errorMsg,
                                   String emptyMsg, int errorIcon, int emptyIcon) {
        ...
        switch (state) {
            ...
            case VIEW_STATE_EMPTY:
                if (emptyIcon > 0)
                    ((ImageView) view.getView(VIEW_STATE_EMPTY).findViewById(R.id.image_icon)).setImageResource(errorIcon);
                if (!StringUtils.isEmpty(emptyMsg))
                    ((TextView) view.getView(VIEW_STATE_EMPTY).findViewById(R.id.text_content)).setText(emptyMsg);
                if (!StringUtils.isEmpty(emptyListener)&&!StringUtils.isEmpty(view.getView(VIEW_STATE_EMPTY).findViewById(R.id.button_retry)))
                    view.getView(VIEW_STATE_EMPTY).findViewById(R.id.button_retry).setOnClickListener(emptyListener);
                break;

            case VIEW_STATE_ERROR:
                if (errorIcon > 0)
                    ((ImageView) view.getView(VIEW_STATE_ERROR).findViewById(R.id.image_icon)).setImageResource(errorIcon);
                if (!StringUtils.isEmpty(errorMsg))
                    ((TextView) view.getView(VIEW_STATE_ERROR).findViewById(R.id.text_content)).setText(errorMsg);
                if (!StringUtils.isEmpty(onClickListener))
                    view.getView(VIEW_STATE_ERROR).findViewById(R.id.button_retry).setOnClickListener(onClickListener);
                break;
